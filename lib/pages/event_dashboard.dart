...
class _EventDashboardPageState extends State<EventDashboardPage> {
  ...
  bool isLoading = false;
    ...
    final res=await NetworkService.sendRequest(path: '/event/dashboard',context: context,body: {'from_date':fromDate,'to_date':toDate});
    print("res $res");
    setState(() {
       isLoading=false;
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: KovilAppBar(),
      body: isLoading? Center(child: CircularProgressIndicator(color: Colors.orange,),) 
      : Column(
        children: [
          SizedBox(height: 20,),
            Row(
